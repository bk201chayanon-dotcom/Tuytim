 ...
 *
 * ผลลัพธ์:
 *   dist/anime-quiz.html        เปิดด้วยเบราว์เซอร์ได้ทันที ส่งต่อให้เพื่อนได้
 *   dist/anime-quiz.embed.html  สำหรับโฮสต์ที่ห่อ <html>/<head>/<body> ให้เองแล้ว
 *
...
fs.writeFileSync(path.join(root, 'dist', 'anime-quiz.html'), html);
...
fs.writeFileSync(path.join(root, 'dist', 'anime-quiz.embed.html'), fragment);

const kb = s => (Buffer.byteLength(s) / 1024).toFixed(1);
console.log(`เขียน dist/anime-quiz.html (${kb(html)} KB) และ dist/anime-quiz.embed.html (${kb(fragment)} KB)`);
