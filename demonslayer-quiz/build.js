 ...
 *
 * ผลลัพธ์:
 *   dist/demonslayer-quiz.html        เปิดด้วยเบราว์เซอร์ได้ทันที ส่งต่อให้เพื่อนได้
 *   dist/demonslayer-quiz.embed.html  สำหรับโฮสต์ที่ห่อ <html>/<head>/<body> ให้เองแล้ว
 *
...
fs.writeFileSync(path.join(root, 'dist', 'demonslayer-quiz.html'), html);
...
fs.writeFileSync(path.join(root, 'dist', 'demonslayer-quiz.embed.html'), fragment);

const kb = s => (Buffer.byteLength(s) / 1024).toFixed(1);
console.log(`เขียน dist/demonslayer-quiz.html (${kb(html)} KB) และ dist/demonslayer-quiz.embed.html (${kb(fragment)} KB)`);
