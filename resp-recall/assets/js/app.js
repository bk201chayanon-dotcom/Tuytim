...
(() => {
  ...
  $('#exportBtn').addEventListener('click', async () => {
    ...
    const filename = 'resp-recall-' + todayKey() + '.json';

    /* แสดงข้อความไว้เสมอ เผื่อบันทึกไฟล์ไม่ได้ก็ยังคัดลอกเก็บเองได้ */
    const box = $('#exportBox');
    box.hidden = false;
    box.value = data;
    ...
    if (!host) {
      try {
        ...
        toast('คัดลอกข้อความในกล่องด้านล่างไปเก็บไว้ได้');
      }
      return;
    }
    ...
    try {
      ...
      else toast('บันทึกไฟล์ไม่สำเร็จ — คัดลอกข้อความในกล่องด้านล่างไปเก็บไว้ได้');
    }
  });
