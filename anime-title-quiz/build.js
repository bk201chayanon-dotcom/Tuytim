 ...
 *
 *   node anime-title-quiz/build.js
 *
 * ผลลัพธ์:
 *   dist/anime-title-quiz.html        เปิดด้วยเบราว์เซอร์ได้ทันที ส่งต่อให้เพื่อนได้
...
fs.writeFileSync(path.join(root, 'dist', 'anime-title-quiz.html'), html);
...
fs.writeFileSync(path.join(root, 'dist', 'anime-title-quiz.embed.html'), fragment);

const kb = s => (Buffer.byteLength(s) / 1024).toFixed(1);
console.log(`เขียน dist/anime-title-quiz.html (${kb(html)} KB) และ dist/anime-title-quiz.embed.html (${kb(fragment)} KB)`);
